...
- **ประวัติของแถว = ตารางใต้แถวนั้น ไม่ใช่เมนู/modal** (แก้ 2026-09-19 — ทับกฎ "เมนูประวัติ 3 ส่วน หัว-list-ท้าย"
  เดิมทั้งข้อ) — badge "แก้ไข n" เป็น **ปุ่ม disclosure** (`aria-expanded`, ไม่มี caret/ไอคอน) กางแถว `colspan`
  เต็มความกว้างต่อท้ายแถวนั้น ข้างในเป็นตาราง **เวลา | ผู้แก้ | จาก → เป็น | ใช้ค่านี้** (โหมดดู = ไม่ render
  คอลัมน์สุดท้าย) — **รายการมีแต่ "การแก้" เท่านั้น ครบทุกแถว ห้ามตัดเหลือ N แถวแรก** เรียงใหม่→เก่า
  (ค่าที่ระบบคำนวณอยู่ใน**แถบหัวแผง** ไม่ใช่แถวของรายการ: มันไม่ใช่ edit ไม่มีเวลา และเป็นฐานที่ทุกแถวเทียบกับมัน
  จึงต้องไม่เลื่อนหาย) — แถวที่เป็นค่าที่ใช้อยู่จริงใส่**คำว่า "ปัจจุบัน" แทนปุ่ม** (ไม่ใช่ปุ่ม
  disabled), ปุ่มที่เหลือเป็น `btn-sm` neutral ตัวเดียวกันทุกแถว (action ที่ซ้ำทุกแถวไม่ใช่ action หลักของหน้า §4),
  หมายเหตุเป็นบรรทัดที่ 2 ใต้ "จาก → เป็น" ไม่กินคอลัมน์เพิ่ม
  - **แถวเก่าที่ค่าซ้ำกับค่าปัจจุบัน = ปุ่ม `disabled`** (ไม่ใช่ "ปัจจุบัน" — คำนั้นเป็นของแถวเดียว) เทียบด้วย
    **ค่าดิบ + tolerance** ไม่ใช่สตริงที่ format แล้ว · ค่าที่ถูก mask ถือว่า "ไม่เท่ากับอะไรเลย" เสมอ ·
    แถว "ค่าที่ระบบคำนวณ" ไม่เข้ากฎนี้ (กดแล้วลบ override จริง ไม่ใช่ no-op)
  - **แผง 3 ชั้น เลื่อนเฉพาะชั้นล่าง**: (ก) แถบหัว = ค่าที่ระบบคำนวณ (tri-state มี 2 ชนิด = 2 บรรทัด ·
    รายการที่เพิ่มเองไม่มีเลย = ซ้ายว่าง) + ปุ่ม/tag ตามกฎเดิม ซ้าย, **ปุ่มปิด = `.btn-close` ตัวเดียวกับหัว modal
    ขวาสุด พื้นที่กด 32px (ได้จาก padding + `--fs-md` เหมือน `.modal-header .btn-close` ไม่ใช่วงกลมมีขอบ —
    แก้ 2026-09-22 slip2-a) ตรงแนวขอบขวาปุ่มในรายการชั้น (ค)** (ชดเชยความกว้าง scrollbar ของชั้น (ค) ด้วย) (ข) หัวคอลัมน์ `sticky top:0`
    พื้นทึบ = พื้นแผง (ค) รายการแก้ scroll เอง **สูงสุด = (ก)+(ข)+5 แถว วัดจากแถวจริง** (แถวที่มีหมายเหตุ
    สูงไม่เท่าแถวเปล่า) · กด × แล้ว `aria-expanded=false` + คืน focus ให้ badge
  - **กรอบซ้อนได้ไม่เกิน 2 ชั้นใน modal เดียว — เกินแล้วตัดชั้นนอกสุด** (2026-09-22 slip2-b) — modal +
    แผง + การ์ดในแผง = 3 ชั้น · ชั้นที่บอกน้อยที่สุดคือชั้นนอก (มันจัดกลุ่มสิ่งที่หัวข้อจัดให้แล้ว) จึงให้การ์ดต่อ
    หัวข้อเก็บ panel rule ไว้ แล้วถอดขอบ/padding ของแผงชั้นนอกออก เหลือแต่ margin
  - **แผงที่กางในหน้าจอเดียวกับของที่ผู้ใช้กำลังเทียบ ต้องจำกัดความสูงตัวเอง** (2026-09-22 slip2-b) —
    เพดาน = **1 แถวของการ์ดที่ render จริง** (วัด ไม่ประกาศค่า) และ `min(..., 45vh)` · scroll box ต้องมี
    `tabindex="0" role="group" aria-label` ไม่งั้นคีย์บอร์ดเลื่อนไม่ได้ (ใช้กับ `.lo-history-scroll` ด้วย)
  - **แผงเป็นกล่องของตัวเอง ไม่ใช่แถบ**: พื้น `--c-bg` + ขอบ `1px --c-border` + `--radius` + **เยื้องซ้าย
    ให้คอลัมน์แรกตรงแนวชื่อรายการของแถวนั้น** (วัดจากหัวคอลัมน์ ไม่ประกาศค่า) — **ห้ามใช้ `--c-bg-subtle`**
    ซึ่งเป็นพื้นของหัวกลุ่ม ไม่งั้นแผงกับหัวกลุ่มอ่านเป็นก้อนเดียวกัน · หัวคอลัมน์ในแผงเงียบด้วย **ตัวอักษร
    muted** พื้นเป็น surface ของแผง (พื้น muted ซ้อนอีกชั้นทำให้หัวอ่านเป็นหัวข้อของตัวเอง)
  - **แผงในแถว `colspan` ที่อยู่ในตารางที่ scroll แนวนอนได้ ต้อง `position:sticky; left:0` + กว้างเท่า
    `clientWidth` ของ scroller ที่ **วัดจริง** (ไม่ใช่ความกว้างตาราง) ไม่งั้นไถหลุดจอไปพร้อม scroll — และต้อง
    `overflow:hidden` ที่แผง เพราะเนื้อหาที่ล้นยังนับเข้า `scrollWidth` ของ scroller แม้จะดันความกว้างไม่ได้
...
**Field ของ `.filter-bar` เป็น `input.form-control` (text/date) ได้ ไม่ต้องเป็น `<select>` เสมอไป**
(2026-09-23, 3e-3b B5) — `initFilterBar()` (app.js) รองรับทั้งคู่ด้วยกติกาเดียวกัน: active/count/chip/
ล้าง ทำงานเหมือนกันทุกอย่าง ตัวอย่างจริง: `#auditLogFilterBar` (ช่วงวันที่, Action History tab)
...
**`serverSide` (opt-in, D2)**: ส่ง `{serverSide:true, ajax:{...}}` เป็น top-level option (ไม่ใช่ซ้อนใน `dtOptions`) — `initSharedDataTable()` ส่ง `serverSide`/`processing`/`ajax` เข้า dtOptions ให้ และย้าย search-box threshold (10 แถวเดิม) จากนับ rowCount ตอนสร้าง เป็นเช็ค `recordsTotal` ทุกครั้งที่ draw แทน (ตารางที่ไม่ส่ง option นี้ไม่ถูกกระทบ) — `mode:'server'` ของ `initExcelColumnFilters()` เพิ่ม option `formatValue(key, value)` (opt-in, D3, `table-column-filter.js`) แยกข้อความที่แสดง/ค้นหา/เรียงในรายการตัวเลือก ออกจากค่าที่ส่งกลับ server (ค่าดิบเสมอ) — ไม่ส่ง option ทั้งสองตัว = พฤติกรรมเดิมทุกจุด
...
- **⋮ ในตาราง = popper `strategy:'fixed'` เสมอ** (`applyFixedStrategyToTableDropdowns()`, app.js — global, ไม่ต้อง wiring ต่อหน้า) — ครอบทั้ง wrapper DataTables 1.x (`.dataTables_wrapper`) และ 2.x (`.dt-container`, เวอร์ชันจริงที่แอปนี้ใช้)
- **ถ้า action ทั้งหมดของแถว ≤ 3 ตัว ห้ามมี ⋮** — ⋮ มีไว้พับส่วนที่เกิน 3 เท่านั้น ไม่มีอะไรเกินก็ไม่ต้องมี (เมนูที่มีรายการเดียว = คลิกสองครั้งขวางหน้า action เดียว) — action ที่มีเงื่อนไข (เช่น draft-only) หายไปเฉยๆ ไม่ต้องมีที่เก็บแทน
- ไอคอนต้องสื่อความหมาย + tooltip เสมอ (BACKLOG: ทบทวนไอคอนทั้งระบบ ทำในรอบ 4)
- **toggle ที่แทน state ของระบบ = tri-state เสมอ (switch เปิด/ปิด + ทางคืนค่าระบบ) ไม่ใช่ checkbox 2 ค่า** — ค่าที่ผู้ใช้ไม่เคยตั้งเอง กับ ค่าที่ตั้งเองแล้วบังเอิญตรงกับค่าระบบ ไม่เหมือนกัน — switch ตอน inherit แสดงค่าที่มีผลจริงโดยไม่มี tag, ตอนถูกตั้งเองมี tag บอกว่าระบบจะตอบว่าอะไร — **แถวที่ toggle ได้ต้อง render เสมอในโหมดแก้ แม้ยอดเป็น 0 หรือระบบบอกว่าไม่เข้าเกณฑ์** (ถ้าซ่อนแถว ก็ไม่มีทางเปิดกลับ); โหมดดูไม่มีอะไรให้ตั้ง จึง render เมื่อมีอะไรจะบอกเท่านั้น
...
- **คอลัมน์ action ที่มีเฉพาะบางโหมดของ modal เดียวกัน = `column(i).visible()` ไม่ใช่ตารางตัวที่สอง** — ดู §9
  "modal ตัวเดียว 2 โหมด"; `<th>` ของคอลัมน์นั้นไม่ต้องมีป้าย และห้ามมี `data-i18n` บน `<th>` ตรงๆ
...
- **ช่องปุ่มท้ายช่องรายการต้องกว้างคงที่เสมอ แม้แถวนั้นจะไม่มีปุ่ม** (render slot เปล่าแทนการไม่ render;
  "ท้ายแถว" เดิม → "ท้ายช่องรายการ" เพราะสลิปย้ายปุ่มเข้ามาอยู่ในเซลล์รายการแล้ว ไม่ใช่คอลัมน์ของตัวเอง —
  2026-09-22 slip2-a · ข้อความแปรผัน เช่น ตัวนับ ให้อยู่**ซ้าย**ของ slot เพื่อให้ขอบขวาคงที่) — ไม่งั้น
...
- **`.btn-icon-ghost` — variant ใหม่ (2026-09-13, filter-bar "ซอฟต์ลง" follow-up)**: ใช้ร่วมกับ
  ...
  - **ขยาย 2026-09-22 (slip2-a): ใช้กับ "ปุ่มที่ซ้ำทุกแถวในรายการ/ตาราง" ด้วย** — ขอบ+พื้นที่ rest คูณ
    จำนวนแถวคือกล่องเพิ่มทีละแถวโดยไม่บอกอะไร (§0.3) ขนาด/พื้นที่กด 32px/hover/focus เหมือนเดิมทุกอย่าง
    (จุดใช้จริง: comment list เดิม + ปุ่มในแถวของสลิป) — ทับคำว่า "แถวในตารางไม่ใช่ panel" ที่เคยเขียนไว้

**ข้อความว่าง**: จาก `getTableLang().emptyTable` — ประโยคเดียวบอกว่าทำอะไรต่อได้ (เช่น "ยังไม่มีรายการ — กด 'เพิ่มรายการ' เพื่อเริ่ม")

**แถบ filter เหนือตาราง (`.filter-bar`, §6) มีไว้เฉพาะสิ่งที่คอลัมน์-header checklist ทำไม่ได้เท่านั้น**
(เช่น ช่วงวันที่ต่อเนื่อง — field เป็น bare `.form-control.datepicker` ได้ ไม่จำเป็นต้องเป็น `<select>`,
2026-09-23 3e-3b) — คอลัมน์ที่เป็นค่าจากลิสต์ปิด (ผู้ทำ/การกระทำ/สถานะ ฯลฯ) ใช้ column filter ของตัวเองอยู่แล้ว
ห้ามทำ select ซ้ำไว้ข้างบนอีก
...
- ขนาด: `modal-lg` เป็นค่าเริ่มต้นสำหรับฟอร์ม, `modal-xl` เฉพาะที่มีตาราง, ไม่ใช้ fullscreen ยกเว้น editor
  - **modal ที่เปิดทับ modal ต้องแคบกว่าตัวล่าง** (2026-09-22 slip2-b) — 2 ตัวที่กว้างเท่ากันอ่านเป็น
    "เปลี่ยนเนื้อในกล่องเดิม" ไม่ใช่หน้าต่างลูก (วัดจริง: สลิปกับฟอร์ม 800 = 800) · ของจริง: สลิป (มีตาราง)
    = `modal-xl`, ฟอร์มบรรทัดที่เปิดทับ = `modal-lg` → 1140 vs 800 · **ห้ามแก้ด้วยการย่อตัวบน** เพราะ
    `.segmented`'s ป้ายสั้นสลับตาม **viewport** ไม่ใช่ความกว้างกล่อง — ฟอร์มที่ถูกบีบที่ 1400 จะได้ป้าย
    เต็มในกล่องที่ไม่พอ
...
- **modal เปิดจากปุ่มแถวในตาราง แสดง record เดียวแบบอ่านอย่างเดียว = ห้ามมีปุ่ม primary ในการกระทำ**
  (footer มีแค่ [ปิด] `btn-outline-secondary`, 2026-09-23 3e-3b) — ใช้ `.rd-sync-field`/`-label`/`-value`
  (§11) เรียงเป็นคอลัมน์เดียวได้ ไม่ต้องมี `.rd-sync-fields` grid ถ้ามีไม่กี่ field
...
- **tab/หน้าที่ยิง API ทันทีทุกการกระทำ = ซ่อนปุ่มบันทึกของ footer และปิด dirty-guard ของ tab นั้น** —
  ...
  ไม่ใช่มีแล้วซ่อน)
- **ดินสอในแถว = เปิดฟอร์มของแถวนั้น ไม่ใช่ inline edit (แก้ 2026-09-18, tiny-L6a — ทับมติ inline edit เดิมทั้งหมด)**:
  เซลล์ไม่กลายเป็น input อีกต่อไป · ปุ่มเปิดแก้ = ไอคอนดินสอ **แสดงตลอด ไม่รอ hover** เปิด **ฟอร์มเดียวกับที่ใช้
  เพิ่ม/แก้แถวของ block เดียวกัน** (ข้อ "ฟอร์มเดียวกันที่เปิดได้จาก 2 ที่ขึ้นไป" ด้านล่าง) · เหตุผล: ช่องแก้ในเซลล์
  ถือได้แค่ตัวเลข ส่วนที่เหลือของแถว (หมายเหตุ, ปลายทางของเงิน) จึงไม่มีทางเข้าจากแถวที่มันเป็นของ
  - **1 แถว = 1 ปุ่ม** — action อื่นของแถวนั้น (เช่น "ใช้ค่าที่ระบบคำนวณ") ย้ายไป slot ซ้ายของ footer ในฟอร์ม
    ซึ่งเป็นที่เดียวที่ค่าทั้ง 2 อยู่บนจอพร้อมกัน — ไม่ใช่ปุ่มกลมตัวที่ 2 ข้างดินสอ
  - **สิทธิ์แก้ต่อ field มาจากที่เก็บจริงของบรรทัดนั้น** — section ที่บรรทัดนั้นไม่มีที่เก็บให้เขียน **ไม่ render**
    ไม่ใช่ render แล้ว `disabled` (§0.3) · เขียนไม่ได้แต่ต้องรู้ = ข้อความ read-only + ลิงก์ไปหน้าที่แก้ได้จริง
  - ระหว่างยิง **ล็อกทั้งตาราง** (การกระทำที่สองจะแข่งกับ recalculate ที่ยังไม่จบ) · ล้มแล้วฟอร์มเปิดค้างพร้อมค่าที่พิมพ์
    และบอกเหตุผลใน callout ของฟอร์ม (§15) ไม่ใช่ toast
  - ฟอร์มที่ต้องยิง 2 endpoint (คนละตารางที่เก็บ) = ยิง**ตามลำดับ ไม่ขนาน** ส่งเฉพาะส่วนที่เปลี่ยนจริง ·
    call หลังล้ม = reload ให้เห็นของที่บันทึกไปแล้ว + คำอธิบายในฟอร์ม + ถ่าย baseline ของ dirty-guard ใหม่
  ...
  **ไม่สร้างฟังก์ชันที่ 3** (ตัดสินใจแล้วรอบ 0)
  ...
  - **ยืนยันแล้วให้รื้อกลับ แบบ opt-in ต่างจากเดิม 3 จุด เพื่อไม่ให้กลับไปสับสนซ้ำ**:
    ...
    4. **Modal ที่โหลดข้อมูลแบบ async หลัง `shown.bs.modal` ต้อง scope baseline เอง (ต่อ container/tab)
       ...
       (พบจริงตอนทำ modal ตั้งค่ารายบุคคลของหน้า Process Detail, Round 3 item 4 batch 1/4 — modal นั้นถูกลบทิ้งแล้ว 2026-09-18 กฎยังใช้)
...
- **ยอดรวม (รวมรายได้/รวมหัก/สุทธิ) เป็น บล็อก 2 คอลัมน์ใต้ตาราง นอก scroll container**
  (ห้ามอยู่ใน element ที่ overflow-x — ที่ 430 ตารางกว้างกว่า host แถวของมันจึงเลื่อนหนีไปกับการลาก) —
  label ตรงข้อความหัวกลุ่ม ตัวเลขชิด `--payslip-inset` จากขอบ host โครงเดียวกันทั้ง 2 โหมด — ทุกตัวเลขมาจากแถว
...
- **1 หัวกลุ่ม = 1 ลิงก์เท่านั้น ไม่มีทั้งคู่**: กลุ่มที่เพิ่มเองได้ = "เพิ่มรายการ", กลุ่มที่ระบบคำนวณเอง = "คืนค่าระบบ" (CSS บล็อกเดียว ตำแหน่ง/inset ตรงกัน)
- ลิงก์ "คืนค่าระบบ" render เฉพาะเมื่อกลุ่มมีแถวที่คืนได้จริง ≥ 1 แถว — ไม่มี = ไม่ render (§0.3) ห้ามทำเป็น disabled
...
**สลิป (ดู/แก้) = ตารางเดียว renderer เดียว** (4a-1, 2026-09-18)
- โหมด "ดู" กับ "แก้" ต้อง render จากฟังก์ชันเดียว **ต่างกันได้แค่ "ไม่ render คอลัมน์"** ห้ามใช้ `d-none`
  ซ่อนคอลัมน์/ปุ่มที่โหมดนั้นกดไม่ได้ (ของที่ซ่อนยังอยู่ใน tab order และยังหลอกให้คนอ่าน DOM เชื่อว่ากดได้)
- **ลำดับ tag ใต้บรรทัดตายตัว**: `งวด n/m · ปลายทาง` → เหตุผลของยอด (ยกเว้น/สูตรคำนวณ) → `ระบบ: x` (ใต้ยอด
  ในช่องเงิน) → `หมายเหตุ: …` → **"แก้ไขแล้ว"/"เพิ่มเอง" (โหมดดูเท่านั้น, 2026-09-19)** — ตัวสุดท้ายเป็นคนละคำกัน
  เสมอ: บรรทัดที่เพิ่มเองไม่เคยมีค่าที่ระบบคำนวณ คนละเรื่องกับยอดที่ระบบคำนวณแล้วมีคนแก้ — โหมดแก้ไม่ต้องมี
  (สวิตช์/ดินสอ/`ระบบ: x` บอกอยู่แล้ว §0.3) — ทุกตัวเป็น `.payslip-line-tag` (`--fs-xs`/`--c-text-muted`) **แสดงเต็ม
  wrap ได้ ห้ามย่อ/ห้ามซ่อนส่วนที่เหลือไว้ใน `title` ให้ hover** (บนมือถือไม่มี hover) — ไม่มีข้อมูล =
  **ไม่ render** ห้ามใส่ `-`
- **`หมายเหตุ:` = note ที่คนพิมพ์ตอนแก้ยอด (`override_note`) เท่านั้น** ห้ามเอา note ภายในของ engine
  (`manually_overridden`/`employee_not_enrolled`/…) มาโชว์ตรงนี้ — อันนั้นเป็นรหัส ไม่ใช่ข้อความให้คนอ่าน
- **ห้ามมีปุ่ม "?" popover ต่อบรรทัด** — คำอธิบายที่ต้อง hover ถึงจะเห็นไม่ใช่คำอธิบาย (§7) ให้เป็น tag
  ข้อความบรรทัดเดียวแทน
- **แถวที่ไม่เข้าเกณฑ์ (ไม่ได้เข้า SSO/กองทุน/ปิดที่บริษัท) ไม่ render เลย** ไม่ซ่อนไว้หลัง collapse "แสดง n แถว
  ที่ซ่อน" และไม่แสดงพร้อม badge เหตุผล — แถวที่ไม่ได้เป็นส่วนหนึ่งของเงินงวดนี้ไม่ใช่แถวของสลิป (ยกเว้นมี override
  ของคนนั้นอยู่ → ยัง render ตามปกติ)
- **modal ลูกสืบสิทธิ์จากหน้าจอที่เปิดมัน** — modal ที่เปิดจากสลิปโหมดดู ต้องไม่ render ปุ่มที่เขียนข้อมูล
  (ไม่ใช่ `disabled`/`d-none`) และ endpoint ที่ปุ่มนั้นยิงต้องมี guard ฝั่ง server ตัวเดียวกับ path อื่น
- **ยอดรวมเป็นกล่องใต้ตาราง นอก scroller** (รวมรายได้/รวมรายการหัก/ยอดจ่ายสุทธิ) — แก้ 2026-09-19 ทับมติเดิม
  ที่ให้เป็นแถวท้ายตาราง: ใต้ `sm` ตารางนี้เป็น scroller แนวนอน แถวของมันจึงไถหลุดจอไปพร้อมกัน (วัดจริง) —
  ยอดรวมไม่ใช่บรรทัดของสลิป แต่เป็นบรรทัดสุดท้ายของสลิป จึงต้องเห็นเต็มเสมอ — ตัวเลขมาจาก payload เดิม
  ห้ามบวกใน JS
- **หน้าดู: tab filter 2 ตัวเหนือตาราง** (`.nav-tabs` §6 ไม่ใช่ segmented/pill) "รายละเอียด" / "รายการที่แก้ไข (n)"
  — `n` เป็นเลขเทาในวงเล็บ ห้ามเป็น badge สี, `n=0` → **ไม่ render `<ul>` เลย** (ไม่ใช่ `d-none`/disabled)
- กด tab = **re-render ตารางเดิมด้วย renderer เดิม ห้ามยิง request ใหม่**, เป็น state ฝั่ง client ไม่ persist (เปิด
  modal ใหม่กลับมา tab แรกเสมอ), กลุ่มที่ว่างหลังกรอง **ไม่ render หัวกลุ่ม**
- **ยอดรวม 3 แถวยังเป็นยอดเต็มเสมอ** แม้กำลังกรอง — **ตัวอักษร tab แรกต้องตรง x กับตัวอักษรหัวคอลัมน์แรก**
  (เยื้องทั้งแถว tab ไม่ใช่เยื้อง link ตัวแรก ขีดล่างจะได้สมมาตร)

**modal ตัวเดียว 2 โหมด** (2026-09-22, n) — รายการเดียวกันที่ถูกกรองคนละแบบ = modal เดิมสลับโหมด
ห้าม clone modal/ตารางตัวที่สอง (กฎ "ซ้ำ = shared" §0.4):
- **ตัวแปรโหมด 1 ตัวต่อ modal** อ่านโดยทุกจุดที่ต่างกัน (title/callout/คอลัมน์/ป้ายปุ่ม) **และโดยทุก endpoint
  ของตารางนั้น** (options, all-ids, column-values ต้องส่งเหมือนกันหมด ไม่งั้น paging กับ filter คนละประชากร)
- **ทุกทางที่เปิด modal ต้องผ่านฟังก์ชัน open ตัวเดียวกัน** ที่รีเซ็ตโหมด+ตัวเลือก+ฟิลเตอร์ทุกครั้ง — โหมดห้าม
  ค้างข้ามการเปิด
- **title สลับด้วย `.attr('data-i18n', key)` บน `<span>` leaf** (ไม่ใช่ `.text()` เปล่า) — sweep ของ
  `updateText()` อ่าน attribute สดทุกครั้ง ภาษาจึงยังถูกหลังสลับโหมด
- **ป้ายที่เป็น template `{count}` ต้องถอด `data-i18n` ออกตอนใช้ template** แล้วให้ `refreshXxxLanguage()`
  เป็นคนวาดใหม่ — ทิ้ง marker ไว้ = สลับภาษาแล้วได้คำว่า `{count}` ดิบบนปุ่ม
- **คอลัมน์เฉพาะโหมด = `column(i).visible()` + `columns.adjust()`** ไม่ใช่ init ตารางใหม่
- **empty state ต่อโหมด = `dtRenderEmptyState(api, config)` ใน `drawCallback`** (§11) ห้ามยัด HTML ลง
  `language.emptyTable` — DataTables อ่าน `language` ครั้งเดียวตอน construct โหมด/ภาษาจะค้างอยู่ในนั้น

...
- **ลำดับ tag ใต้บรรทัดตายตัว**: `งวด n/m · ปลายทาง` → เหตุผลของยอด (ยกเว้น/สูตรคำนวณ) → `ระบบ: x` (ใต้ยอด
  ในช่องเงิน) → `หมายเหตุ: …` — ทุกตัวเป็น `.payslip-line-tag` (`--fs-xs`/`--c-text-muted`) **แสดงเต็ม
...
- **ยอดรวมเป็นแถวท้ายตารางเดียวกัน** (รวมรายได้/รวมรายการหัก/ยอดจ่ายสุทธิ) ไม่ใช่กล่องใต้ตาราง — ตัวเลขมาจาก
  payload เดิม ห้ามบวกใน JS

...
<div class="cp-section">
    ...
    <p class="cp-section-note"><b>select2 โหมด ajax -- 2 option ใหม่ (2026-09-17, R1b) ทั้งคู่เป็น opt-in ต่อช่อง</b> ไม่มีผลกับช่องที่ไม่ส่งมา (สาธิตสดที่นี่ไม่ได้ ทั้งคู่ต้องมี endpoint + session จริง):
        <b>(1) <code>pinnedOption: {id, key, fallback}</code></b> -- ตัวเลือกตายตัว 1 ตัวที่ endpoint ไม่มีวันคืนมา วางท้ายสุด มีเส้นคั่นด้านบน (<code>.select2-pinned-option</code> ใน style.css) ติดมากับ<b>หน้าสุดท้าย</b>เท่านั้น (ไม่ซ้ำตอนเลื่อน) และติดมาทุกคำค้น (ทางออกต้องกดถึงได้เสมอแม้ค้นแล้วไม่เจออะไรเลย) -- ใช้กับ "ตัวเลือกที่ไม่ได้มาจากข้อมูล" เช่น "อื่นๆ (ระบุชื่อ)" ของฟอร์มเพิ่มรายการใน Payroll Run Detail.
        <b>(2) <code>stripCodePrefix: true</code></b> -- endpoint ที่คืน label มาเป็น <code>[CODE] ชื่อ</code> อยู่แล้ว: ตัด <code>[CODE] </code> ออกจากข้อความที่แสดง แล้วเอา code ไปเป็น <code>title</code> ของ option (select2 ใส่ <code>data.title</code> ให้ทั้ง <code>&lt;li&gt;</code> ในดรอปดาวน์และกล่องที่ปิดอยู่) -- ตรงกฎ "รหัสภายในไม่พิมพ์ inline ไปอยู่ที่ title" (§5/§6). <b>การค้นหาไม่กระทบ</b>: โหมด ajax ค้นที่ server และ query ของ endpoint พวกนี้ match ทั้ง <code>item_code</code> และชื่อ th/en อยู่แล้ว -- พิมพ์ code ยังเจอแถวเดิม แค่ไม่มี option ไหนพิมพ์ code ออกมาให้เห็น. ตัวแยกคือ <code>splitOptionCodePrefix(text)</code> (app.js) -- label ที่ไม่มี <code>[...]</code> นำหน้าคืนกลับมาเหมือนเดิม code ว่าง; ตัดทั้งที่ <code>processResults</code> และที่ <code>templateResult</code>/<code>templateSelection</code> เพื่อให้ option ที่ไม่ได้มาจาก ajax (เช่น prefill ด้วย <code>new Option()</code>) แสดงชื่ออย่างเดียวเหมือนกัน.
        <b>(3) option ทั้ง 2 ตัวรอดจากการ re-init แล้ว (บั๊กจริง 2026-09-17, วัดในเบราว์เซอร์)</b> -- <code>applyLanguage()</code> (app.js) วน <code>initSelect2Remote($field)</code> ใส่ทุก <code>.select2-remote</code> บนหน้า <b>โดยไม่ส่ง option ใดๆ</b> ทุกครั้งที่โหลดหน้าและทุกครั้งที่สลับภาษา ของที่ field ขอไว้ตอน init ของตัวเองจึงถูกทิ้งเงียบๆ มาตลอด (ที่เจอจริง: <code>allowClear</code> ของ picker 3 ตัวในหน้า Payroll Run Detail) -- <code>initSelect2()</code> จำ option ไว้ที่ตัว element แล้ว merge ไว้ใต้ค่าที่ส่งมาใหม่ (<code>selectedValue</code> ไม่จำ เป็นคำสั่งครั้งเดียว) <b>ห้ามเรียก <code>.select2({...})</code> ตรงๆ จาก page script</b> -- จะโดน re-init ลบทิ้ง.</p>
</div>
    ...
    <div class="row g-3">
        <div class="col-sm-2">
            <!-- 2026-09-13, real bug repro fix: MUST be a genuine select2-remote (ajax) field, matching
                 employee/list.php's own #employee_filter_department EXACTLY (same class, same
                 data-api/data-type -- 2026-09-23 round B1, มติ ข: the original reference here pointed at
                 a payroll/detail.php field that was removed that round (see docs/decisions); this demo
                 field's own config is unchanged, only which still-real field it's described as matching) --

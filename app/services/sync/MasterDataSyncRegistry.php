...
/**
 * Fixed dependency order per the sync requirement: department -> position -> shift -> holiday ->
 * leave_type -> ot_rate -> employee (employees resolves department/position/shift refs, so those
 * three must be synced first; holiday/leave_type/ot_rate have no cross-dependency on each other
 * or on employees, so their relative order among themselves doesn't matter functionally -- kept
 * in the order the requirement listed them).
 *

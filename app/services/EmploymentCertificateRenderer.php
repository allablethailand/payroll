...
class EmploymentCertificateRenderer {
    ...
    // with PayslipTemplateRenderer, extracted after verifying every method body-for-body identical
     ...
     *  image library -- note the table name and upload subdir are NOT the same string for this
     *  class (a real, easy-to-miss distinction preserved exactly as it already existed, not
     *  flattened). See that trait's own docblock for why these are abstract requirements instead of
     *  a hardcoded value shared with PayslipTemplateRenderer. */
    protected function imageLibraryTableName(): string {
        return 'employment_certificate_images';
    }
    protected function imageLibraryUploadSubdir(): string {
        return 'employment_cert_images';
    }
    ...
    /** needsSymbolFontFallback()/pageDimensionsMm() now live in PdfCanvasRendererTrait (identical
     *  body, verified before extraction) -- pageDimensionsMm() is still callable as
     *  EmploymentCertificateRenderer::pageDimensionsMm(...) exactly as before, PHP trait methods
     *  are indistinguishable from the class's own from the outside. */
     ...
     *  PayslipTemplateRenderer's own copy, parameterized via imageLibraryTableName()/

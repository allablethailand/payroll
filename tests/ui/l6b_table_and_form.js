...
async function runCell(opts) {
    ...
    // 2 -- "ระบบ: x" only under a row that really carries an override.
    ...
    const addBtn = await page.evaluate(() => {
        const b = document.querySelector('#breakdownManualLines .manual-line-add-btn');
        if (!b) return null;
        ...
        return {
            ...
            count: document.querySelectorAll('#breakdownManualLines .manual-line-add-btn').length,
        };
    });
    ...
    await page.locator('#breakdownManualLines .manual-line-add-btn').first().click();
    await page.waitForSelector(`${FORM}.show`, { timeout: 15000 });
    await page.waitForTimeout(600);
...
async function runWriteCell() {
    ...
    if (await page.locator(`${WRAP} tr.lo-row[data-item-code="${recurringCode}"]`).count() === 1) {
        ...
        check(`${label}: the amount field holds what was typed`, typed.replace(/[^\d]/g, '') === '77700', typed);
        ...
        await page.waitForTimeout(800);
        await page.locator('.select2-results__option[role="option"]:not(.select2-results__message)').first().click();
        ...
        await page.waitForTimeout(2500);
        check(`${label}: the save sent exactly 2 requests`, posted.length === 2, JSON.stringify(posted));
        check(`${label}: the amount goes first, the destination second`,
            posted[0] === 'payroll-run.line-override.save'
            && posted[1] === 'payroll-run.recurring-deduction-destination-override.save', JSON.stringify(posted));
        const after = await page.getAttribute(`${WRAP} tr.lo-row[data-item-code="${recurringCode}"]`, 'data-amount');
        check(`${label}: the table shows the new figure`, after !== before && after.indexOf('777') !== -1, `${before} -> ${after}`);
    ...
    const offersUse = await page.locator('#btnLineFormUseComputed:visible').count();
    check(`${label}: an overridden row offers "use the calculated value"`, offersUse === 1, String(offersUse));
...
async function runEnCell() {
    ...
    const strings = await page.evaluate((wrap) => {
        ...
        const add = document.querySelector('#breakdownManualLines .manual-line-add-btn');

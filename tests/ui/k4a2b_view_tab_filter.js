    ...
    return page.evaluate((wrap) => {
        ...
        return {
            ...
            changedRows: rows.filter(r => r.className.indexOf('lo-row-manual') !== -1
                || (r.getAttribute('data-orig-action') || '') !== '').length,
...
async function runCell(opts) {
    ...
    check(`${label}: the second tab carries the count in brackets`,
        /\(2\)$/.test(all.tabTexts[1] || ''), JSON.stringify(all.tabTexts));
    ...
    check(`${label}: the filtered table renders exactly the changed rows`,
        changed.rowCount === 2 && changed.changedRows === 2, `${changed.rowCount}/${changed.changedRows}`);
    check(`${label}: ...and no group head is left standing over nothing`,
        changed.emptyGroups === 0 && changed.groupCount > 0 && changed.groupCount < all.groupCount,
    ...
    check(`${label}: the same rows in both slips`, edit.rowCount === all.rowCount, `${edit.rowCount} vs ${all.rowCount}`);

...
try {
    ...
    check('all 13 defaults inserted on a fresh company', $firstRes['inserted'], 13);
    ...
    check('13 rows actually exist for this company', count($rows), 13);
    ...
    check('all 13 reported as already existing', $secondRes['skipped'], 13);
    $rowsAfterSecond = (int)$pdo->query("SELECT COUNT(*) FROM payroll_earning_deduction_types WHERE comp_id = {$compId}")->fetchColumn();
    check('still exactly 13 rows (no duplicates created)', $rowsAfterSecond, 13);
    ...
    check('re-seeding after a manual delete does not resurrect the deleted item (still counted as existing/skipped)', $thirdRes['skipped'], 13);
    ...
    check('12 items remain active after deleting 1 of the 13', $activeCount, 12);
} catch (Throwable $e) {
    $failures++;
